...
class attackerAction():
    ...
    def getFeatures(self, gameState, action):
        ...
        capsulesList = self.agent.getCapsules(successor)
    ...
    def chooseAction(self, gameState):
        ...
        simuResult = []
        for action in actions:
            value = self.simulation(2, gameState.generateSuccessor(self.agent.index, action), 0.9)
...
class Attacker(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        if self.getScore(gameState) > 5:
            action = self.defendStatus.chooseAction(gameState)
        else:
            action =  self.attackStatus.chooseAction(gameState)
